...
{	
	...
	void checkVulkanLimits(VkPhysicalDevice device) {
	...
	*/
	VulkanDevice::VulkanDevice(VkPhysicalDevice physicalDevice)
	{
		...
		checkVulkanLimits(physicalDevice);
	}
	...
	*/
	VkResult VulkanDevice::createLogicalDevice(VkPhysicalDeviceFeatures enabledFeatures, std::vector<const char*> enabledExtensions, void* pNextChain, bool useSwapChain, VkQueueFlags requestedQueueTypes)
	{
		...
		std::vector<float> defaultQueuePriorities{ 0.0f, 0.0f };
		...
		// Create a logical device as a connection to a physical device.
	...
	*/
	...
	void VulkanDevice::copyBuffer(vks::Buffer* src, vks::Buffer* dst, VkQueue queue, VkCommandBufferUsageFlagBits flags, VkBufferCopy* copyRegion)
	{
		assert(dst->size <= src->size);
		assert(src->buffer);
		VkCommandBuffer copyCmd = createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true, flags);
		VkBufferCopy bufferCopy{};
		if (copyRegion == nullptr)
		{
			bufferCopy.size = src->size;
	...
	*/
	...
	// extended version for using flags of command buffer
	VkCommandBuffer VulkanDevice::createCommandBuffer(VkCommandBufferLevel level, VkCommandPool pool, bool begin, VkCommandBufferUsageFlagBits flags)
	{
		VkCommandBufferAllocateInfo cmdBufAllocateInfo = vks::initializers::commandBufferAllocateInfo(pool, level, 1);
		VkCommandBuffer cmdBuffer;
		VK_CHECK_RESULT(vkAllocateCommandBuffers(logicalDevice, &cmdBufAllocateInfo, &cmdBuffer));
	...
	VkCommandBuffer VulkanDevice::createCommandBuffer(VkCommandBufferLevel level, bool begin, VkCommandBufferUsageFlagBits flags)
	{
		return createCommandBuffer(level, commandPool, begin, flags);
	}
	...
	*/
	...
	VkResult VulkanDevice::createAndCopyToDeviceBuffer(void* data, vks::Buffer& buffer, size_t bufferSize, VkQueue& queue, VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryFlags) {
		...
		VK_CHECK_RESULT(createBuffer(
			static_cast<VkBufferUsageFlags>(VK_BUFFER_USAGE_TRANSFER_DST_BIT | usageFlags),
			static_cast<VkMemoryPropertyFlags>(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | memoryFlags),
			&buffer,
			bufferSize,
			nullptr));
