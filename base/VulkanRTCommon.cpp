...
void VulkanRTCommon::setupRenderPass()
{
...
void VulkanRTCommon::setupFrameBuffer()
{
...
void VulkanRTCommon::enableExtensions()
{
...
void VulkanRTCommon::deleteScratchBuffer(ScratchBuffer& scratchBuffer)
{
...
void VulkanRTCommon::createAccelerationStructure(AccelerationStructure& accelerationStructure, VkAccelerationStructureTypeKHR type, VkAccelerationStructureBuildSizesInfoKHR buildSizeInfo)
{
	// Buffer and memory
	VkBufferCreateInfo bufferCreateInfo{};
	bufferCreateInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
...
void VulkanRTCommon::deleteAccelerationStructure(AccelerationStructure& accelerationStructure)
{
...
uint64_t VulkanRTCommon::getBufferDeviceAddress(VkBuffer buffer)
{
...
void VulkanRTCommon::createStorageImage(VkFormat format, VkExtent3D extent)
{
...
void VulkanRTCommon::createStorageImage(StorageImage &image, VkFormat format, VkExtent3D extent)
{
...
void VulkanRTCommon::deleteStorageImage()
{
...
void VulkanRTCommon::deleteStorageImage(StorageImage& image)
{
...
void VulkanRTCommon::prepare()
{
	...
	if (!rayQueryOnly) {
		...
		deviceProperties2.pNext = VK_NULL_HANDLE;
	}
...
VkStridedDeviceAddressRegionKHR VulkanRTCommon::getSbtEntryStridedDeviceAddressRegion(VkBuffer buffer, uint32_t handleCount)
{
...
void VulkanRTCommon::drawUI(VkCommandBuffer commandBuffer, VkFramebuffer framebuffer)
{
	...
	VulkanRTBase::drawUI(commandBuffer);
	vkCmdEndRenderPass(commandBuffer);
}

void VulkanRTCommon::drawUI(VkCommandBuffer commandBuffer, VkFramebuffer framebuffer, const vks::Buffer vertexBuffer, const vks::Buffer indexBuffer)
{
	VkClearValue clearValues[2];
	clearValues[0].color = defaultClearColor;
	clearValues[1].depthStencil = { 1.0f, 0 };
	...
	vkCmdBeginRenderPass(commandBuffer, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
	VulkanRTBase::drawUI(commandBuffer, vertexBuffer, indexBuffer);
	vkCmdEndRenderPass(commandBuffer);
}
