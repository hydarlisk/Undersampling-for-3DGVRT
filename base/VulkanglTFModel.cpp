...
bool loadImageDataFunc(tinygltf::Image* image, const int imageIndex, std::string* error, std::string* warning, int req_width, int req_height, const unsigned char* bytes, int size, void* userData)
{
	...
	if (image->uri.find_last_of(".") != std::string::npos) {
		if (image->uri.substr(image->uri.find_last_of(".") + 1) == "ktx" || image->uri.substr(image->uri.find_last_of(".") + 1) == "ktx2") {
			return true;
		}
	}
...
bool formatSupported(VkFormat format, vks::VulkanDevice* device)
{
...
vkglTF::CompressionFormat getAvailableTargetFormat(vks::VulkanDevice* device)
{
	std::vector<vkglTF::CompressionFormat> available_target_formats;

	// Block compression
	if (device->features.textureCompressionBC)
	{
		// BC7 is the preferred block compression if available
		if (formatSupported(VK_FORMAT_BC7_UNORM_BLOCK, device))
		{
			available_target_formats.push_back({ KTX_TTF_BC7_RGBA , VK_FORMAT_BC7_UNORM_BLOCK });
		}

		if (formatSupported(VK_FORMAT_BC3_UNORM_BLOCK, device))
		{
			available_target_formats.push_back({ KTX_TTF_BC3_RGBA, VK_FORMAT_BC3_UNORM_BLOCK });
		}
	}
	...
	{
		if (formatSupported(VK_FORMAT_ASTC_4x4_UNORM_BLOCK, device))
		{
			available_target_formats.push_back({ KTX_TTF_ASTC_4x4_RGBA, VK_FORMAT_ASTC_4x4_UNORM_BLOCK });
		}
	}
	...
	{
		if (formatSupported(VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK, device))
		{
			available_target_formats.push_back({ KTX_TTF_ETC2_RGBA, VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK });
		}
	}

	if (available_target_formats.size() == 0)
		assert("GPU doesn't support Compression!");

	vkglTF::CompressionFormat target_format;
	target_format = available_target_formats[0];
	for (auto current_format : available_target_formats)
	{
		if (current_format.ktxFomrat == KTX_TTF_ASTC_4x4_RGBA)
		{
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, std::string path, vks::VulkanDevice* device, VkQueue copyQueue, vkglTF::CompressionFormat targetFormat)
{
	this->device = device;
	...
	if (gltfimage.uri.find_last_of(".") != std::string::npos) {
		...
		else if (gltfimage.uri.substr(gltfimage.uri.find_last_of(".") + 1) == "ktx2") {
			isKtx = 2;
		}
	}
	...
	else
	{
		...
		AAsset_read(asset, textureData, size);
		AAsset_close(asset);
		result = ktxTexture2_CreateFromMemory(textureData, size, KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &ktx_texture);
		delete[] textureData;
#else
		if (!vks::tools::fileExists(filename)) {
			vks::tools::exitFatal("Could not load texture from " + filename + "\n\nMake sure the assets submodule has been checked out and is up-to-date.", -1);
		}
		result = ktxTexture2_CreateFromNamedFile(filename.c_str(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &ktx_texture);
#endif		
		assert(result == KTX_SUCCESS);
		...
		if (ktxTexture2_NeedsTranscoding(ktx_texture))
		{
			...
			result = ktxTexture2_TranscodeBasis(ktx_texture, targetFormat.ktxFomrat, 0);
		...
		// @todo: Use ktxTexture_GetVkFormat(ktxTexture)
		format = targetFormat.vulkanFormat;
		...
		VkBufferCreateInfo bufferCreateInfo = vks::initializers::bufferCreateInfo();
		bufferCreateInfo.size = ktx_texture->dataSize;
		// This buffer is used as a transfer source for the buffer copy
		bufferCreateInfo.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
		bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		VK_CHECK_RESULT(vkCreateBuffer(device->logicalDevice, &bufferCreateInfo, nullptr, &stagingBuffer));
		...
		for (uint32_t i = 0; i < mipLevels; i++)
		{
			...
			bufferCopyRegion.imageSubresource.baseArrayLayer = 0;
			bufferCopyRegion.imageSubresource.layerCount = 1;
			bufferCopyRegion.imageExtent.width = std::max(1u, ktx_texture->baseWidth >> i);
			bufferCopyRegion.imageExtent.height = std::max(1u, ktx_texture->baseHeight >> i);
			bufferCopyRegion.imageExtent.depth = 1;
			bufferCopyRegion.bufferOffset = offset;
			bufferCopyRegions.push_back(bufferCopyRegion);
		}
...
glm::mat4 vkglTF::Node::localRotationMatrix()
{
	return glm::mat4(rotation) * matrix;
}

glm::mat4 vkglTF::Node::getRotationMatrix()
{
	glm::mat4 m = localRotationMatrix();
	vkglTF::Node* p = parent;
	while (p) {
		m = p->localRotationMatrix() * m;
		p = p->parent;
	}
	return m;
}
...
void vkglTF::Model::loadNode(vkglTF::Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
{
	...
	if (node.mesh > -1) {
		...
		for (size_t j = 0; j < mesh.primitives.size(); j++) {
			...
			//Y&Y added begin
			newMesh->objectID = (this->primitiveNum)++;
			//Y&Y added end
			...
			// Vertices
			{
				...
				for (size_t v = 0; v < posAccessor.count; v++) {
					...
					//Y&Y added begin
					vert.objectID.data = newMesh->objectID;
					//Y&Y added end
	...
	if (!node.extensions.empty()) {
		for (auto ext : node.extensions) {
			if (ext.first == "KHR_lights_punctual") {
				int lightIdx = ext.second.Get("light").GetNumberAsInt();

				//[prev]
				//if (lights[lightIdx].type == DIRECTIONAL_LIGHT)
				//	lights[lightIdx].matrix = newNode->getRotationMatrix();
				//else

				lights[lightIdx].matrix = newNode->getMatrix();	// Suppose that the extension property belongs to children. (by Sehee)
			}
			else {	// for other extensions.
...
{
	vkglTF::CompressionFormat targetFormat = getAvailableTargetFormat(device);
	for (tinygltf::Image& image : gltfModel.images) {
		vkglTF::Texture texture;
		texture.fromglTfImage(image, path, device, transferQueue, targetFormat);
...
void vkglTF::Model::loadMaterials(tinygltf::Model& gltfModel)
{
	for (tinygltf::Material& mat : gltfModel.materials) {
		...
		if (mat.values.find("extras") != mat.values.end()) {
			material.Kr = static_cast<float>(mat.values["extras"].json_double_value["Kr"]);
			material.Kt = static_cast<float>(mat.values["extras"].json_double_value["Kt"]);
		}
		else if (mat.extras.Has("Kr"))
		{
			material.Kr = static_cast<float>(mat.extras.Get("Kr").Get<double>());
			material.Kt = static_cast<float>(mat.extras.Get("Kt").Get<double>());
		}
		if (mat.extensions.find("KHR_materials_ior") != mat.extensions.end()) {
			tinygltf::Value extension = mat.extensions["KHR_materials_ior"];
			material.ior = static_cast<float>(extension.Get("ior").GetNumberAsDouble());
		}
...
void vkglTF::Model::loadLights(tinygltf::Model& gltfModel)
{
	for (tinygltf::Light& li : gltfModel.lights) {
		vkglTF::Light light(device);
		
		// 1. Color 
		if (li.color.size() > 0) {
			light.color = glm::vec3(li.color[0], li.color[1], li.color[2]);
		}
		else {
		...
		// 2. Type
		if (li.extras.Has("type"))
		{
			if (li.extras.Get("type").Get<std::string>() == "static")
				light.type = STATIC_LIGHT;
			else
				light.type = DYNAMIC_LIGHT;
		}
		...
		//	if (!firstPointLight) {
		//		pointLightOffset = lights.size();
		//		firstPointLight = true;
		//	}
		//}
		...
		//	light.spot.innerConeAngle = li.spot.innerConeAngle;
		//	light.spot.outerConeAngle = li.spot.outerConeAngle;
		//}
		...
		// 4. Radius
		if (li.extras.Has("radius"))
			light.radius = static_cast<float>(li.extras.Get("radius").Get<double>());
...
void vkglTF::Model::saveGeometries(std::vector<Vertex>& vertexBuffer, std::vector<uint32_t>& indexBuffer, VkQueue transferQueue) {
...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	gltfContext.SetStoreOriginalJSONForExtrasAndExtensions(true);
#if defined(__ANDROID__)
	...
	saveGeometries(vertexBuffer, indexBuffer, transferQueue);
