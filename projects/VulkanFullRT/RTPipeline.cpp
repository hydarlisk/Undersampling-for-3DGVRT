...
void RTPipeline::createDescriptorSets() {
	std::vector<VkDescriptorPoolSize> poolSizes = {
		...
		vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3 * swapchainImageCnt),
...
void RTPipeline::record(VkCommandBuffer& commandBuffer, VkStridedDeviceAddressRegionKHR& raygen, VkStridedDeviceAddressRegionKHR& miss, VkStridedDeviceAddressRegionKHR& hit, uint32_t imageIndex, uint32_t width, uint32_t height, bool additionalRTFlag) {
	if (additionalRTFlag) {
	...
	vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_RAYGEN_BIT_KHR, 0, sizeof(int), &additionalRTFlag);
